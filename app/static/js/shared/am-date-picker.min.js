(function()
{
    ...
    function amDatePicker() {
        return {
            restrict: 'AE',
            templateUrl: '/static/partials/am-date-picker.html',
            require: ['ngModel'],
            scope:
            {
                ngModel: '=',
    ...
    function AmDatePickerController($scope, $timeout, $mdDialog, amDatePickerConfig) {
        ...
        $scope.$watch(
            "amDatePicker.ngModel",
            function(newValue, oldValue){

                var t1 = moment(amDatePicker.ngModel).format(amDatePicker.inputDateFormat);
                var t2 = amDatePicker.ngModelMoment.format(amDatePicker.inputDateFormat);
                if(t1 != t2 && amDatePicker.ngModel != null){
                    amDatePicker.select(moment(newValue));
                }else if(amDatePicker.ngModel == null){
                   init()
                }
                //generateCalendar();
            }
        ...
        function init() {
            ...
            amDatePicker.ngModelMoment = (angular.isDefined(amDatePicker.ngModel) && amDatePicker.ngModel !== null) ?
                moment(angular.copy(amDatePicker.ngModel)) : moment();

            amDatePicker.ngModelMomentFormatted = (angular.isDefined(amDatePicker.ngModel) && amDatePicker.ngModel !== null) ?
        ...
        function clearDate()
        {
            amDatePicker.ngModel = null;
            amDatePicker.ngModelMomentFormatted = "";
        }
        ...
        function select(_day) {
            if (!_day.disabled) {
                amDatePicker.ngModel = _day.isValid() ? _day.toDate() : moment().toDate();
                amDatePicker.ngModelMoment = _day.isValid() ? angular.copy(_day) : moment();
                amDatePicker.ngModelMomentFormatted = _day.isValid() ? _day.format(amDatePicker.inputDateFormat) : "";
                generateCalendar();
            }
        }
